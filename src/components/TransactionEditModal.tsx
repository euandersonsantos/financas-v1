      ...
      >
        <div className={`fixed bottom-0 left-0 right-0 bg-white rounded-t-[24px] p-4 pb-6 z-[61] transition-all duration-300 ease-out ${isClosing ? 'translate-y-full' : 'translate-y-0'} ${!isClosing && isOpen ? 'animate-slide-in-bottom' : ''}`} onClick={e => e.stopPropagation()}>
          ...
          <div className={`transition-all duration-300 ease-in-out ${currentScreen === 'value' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full absolute'}`}>
            {/* Value Edit Screen */}
            <div className="mt-8 w-full"> {/* Reduced margin-top */}
              <h1 className="text-xl font-semibold text-gray-800 mb-1">
              ...
              {/* Value Input */}
              <div className="mb-6"> {/* Removed duplicate div */}
              ...
              <div className={`transition-all duration-300 ease-in-out ${currentScreen === 'value' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full absolute'} absolute bottom-0 left-0 right-0 px-4 pt-4 pb-6 bg-white z-[62]`}> {/* Added z-index and positioning classes, adjusted padding */}
